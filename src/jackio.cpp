...
#include "../include/aubio_priv.hpp"
    ...
    uint_t imidichan, uint_t omidichan)
{
  aubio_jack_t *jack_setup = aubio_jack_alloc (ichan, ochan,
      imidichan, omidichan);
  uint_t i;
  std::string client_name = "aubio";
  char *jack_port_type;
  char name[64];
  /* initial jack client setup */
  jack_options_t options = JackNullOption;
  jack_status_t *status = NULL;
  if ((jack_setup->client = jack_client_open (client_name.c_str(), options, status)) == 0) {
  ...
  if (jack_setup->omidichan) {
    ...
    if (jack_setup->midi_out_ring == NULL) {
      std::cout << "Failed creating jack midi output ringbuffer." << std::endl;
      AUBIO_QUIT (AUBIO_FAIL);
    }
...
beach:
  std::cout << "failed registering port " << client_name << name << std::endl;
...
aubio_jack_midi_event_write (aubio_jack_t * dev, jack_midi_event_t * event)
{
  ...
  if (jack_ringbuffer_write_space (dev->midi_out_ring) < sizeof (*event)) {
    std::cout << "Not enough space to write midi output, midi event lost!" << std::endl;
    return;
  }
  ...
  if (written != sizeof (*event)) {
    std::cout << "Call to jack_ringbuffer_write failed, midi event lost!" << std::endl;
  }
}
...
process_midi_output (aubio_jack_t * dev, jack_nframes_t nframes)
{
  ...
  if (port_buffer == NULL) {
    std::cout << "Failed to get jack midi output port, will not send anything" << std::endl;
    return;
  }
  ...
  while (jack_ringbuffer_read_space (dev->midi_out_ring)) {
    ...
    if (read != sizeof (ev)) {
      std::cout << "Short read from the ringbuffer, possible note loss." << std::endl;
    ...
    if (buffer == NULL) {
      std::cout << "Call to jack_midi_event_reserve failed, note lost." << std::endl;
      break;
    }
